...
dataset = pd.read_csv('Churn_Modelling.csv') #read the file
X_Input_data = dataset.iloc[:, 3:-1] #decide which columns (attributes) will be used in the calculations of the ANN
#the values from the column index 3 to the one before last
...
"""Initializing the Artifitial Neural Network"""
ANN = tf.keras.models.Sequential()

"""Adding input layer and hidden layers"""
#the input layer will automatically be the the columns of the dataset

ANN.add(tf.keras.layers.Dense(units=14, activation='relu'))
# 'units' is the numbers of neurons and activation is the formula that is being used in the layer
...
#if they were more possible results, they would be one hot encoded and each result would get a neuron
...
#in this case: ['three-digit-variable for Geography', CreditScore, Gender code, Age, ... , Estimated Salary]
#result close to 0: the customer will not leave the bank

#make sure to use the scaling that was used during the training
# use 'transform' to use the same settings as the training
# 'fit_transform' would create new standards

"""From probability to yes/no"""
...
test_results = np.concatenate((y_pred,y_test), axis=1) #concatenate predictions and real resuults in a 2D array
# array([[pred, real],...])
...
"""The code below basically does the same as 'confusion_matrix' and 'accuracy_score' """
SS = LL = SL = LS = 0
